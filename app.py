...
from downloader import download_video, save_cookies, cookies_loaded
...
with st.sidebar:
    ...
    # COOKIES
    st.markdown("**🍪 COOKIES DE YOUTUBE**")
    status_color = "#00ff88" if cookies_loaded() else "#FF10F0"
    status_text = "✓ CARGADAS" if cookies_loaded() else "✗ NO CARGADAS"
    st.markdown(f"<p style='color:{status_color}; font-size:11px;'>{status_text}</p>", unsafe_allow_html=True)
    uploaded_cookies = st.file_uploader("cookies.txt", type=["txt"], label_visibility="collapsed")
    if uploaded_cookies:
        save_cookies(uploaded_cookies.read())
        st.success("✓ Cookies guardadas")
        st.rerun()
    st.markdown("---")
